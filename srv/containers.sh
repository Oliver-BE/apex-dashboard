...
sudo docker run --name shiny -d --expose 3838 --env "VIRTUAL_HOST=www.apexdashboard.ml" --env "VIRTUAL_PORT=3838" --env "LETSENCRYPT_HOST=www.apexdashboard.ml" --env "LETSENCRYPT_EMAIL=carusoisaac@gmail.com" -v /srv/shinyapps/:/srv/shiny-server/ -v /srv/shinylog/:/var/log/shiny-server/ shiny-server # --volumes-from r-data
